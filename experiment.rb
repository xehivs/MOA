...
if ARGV[0] && ARGV[0] == "count"
	puts experimentsCount
else
	...
	for budget in budgetSet
		...
		if File.exists?"results/#{outputFilename}"
			puts "File exists!".red
			if !File.size?"results/#{outputFilename}"
				puts "But it's empty. Processing anyway".yellow
			else
				next
			end
		end
